...
class moduleissues extends RootModule
{
	var $dbinfo = null;
	var $curFilePath = "";
	...
	function getTItleForFooter()
	{
		global $lang, $main, $mysql, $query;
		
		$a = $mysql->mqm( "SELECT * FROM `".$mysql->t_prefix."issues` WHERE `view`=1 ORDER BY `order` DESC LIMIT 5" );
		while( $r = @mysql_fetch_assoc( $a ) ) {
			$t .= "
				<div>
					<a href=\"".$mysql->settings['local_folder'].$this->dbinfo['local']."/".$r['link'].".html\">".$r['title']."</a>
				</div>
			";
		}
		
		return "<h3 class='title'>".$this->getName()."</h3>".$t;
	}
	...
	function getContent( $noname = false )
	{
		...
		if( $query->gp( $this->dbinfo['local'] ) && $r = $mysql->mq( "SELECT * FROM `".$mysql->t_prefix."issues` WHERE `link`='".str_replace( ".html", "", strtolower( $query->gp( $this->dbinfo['local'] ) ) )."' AND `view`=1" ) )
			$selected = $r;
		
		foreach( $query->newGET as $qValue => $v ) {
			$r = $mysql->mq( "SELECT * FROM `".$mysql->t_prefix."issues` WHERE `link`='".str_replace( ".html", "", strtolower( $qValue ) )."' AND `view`=1" );
			if( $r )
				$selected = $r;
		}
		
		$subs = '';
		$a = $mysql->mqm( "SELECT * FROM `".$mysql->t_prefix."issues` WHERE `view`=1 ORDER BY `order` DESC" );
		while( $r = @mysql_fetch_assoc( $a ) ) {
			$subs .= "
		...
		if( $selected ) {
			$main->templates->setTitle( strip_tags( $selected['title'] ), true );
		
			$main->modules->gmi( 'metas' )->updateMeta( 'og:url', "https://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'] );
			$main->modules->gmi( 'metas' )->updateMeta( 'og:title', $selected['title'] );
			$main->modules->gmi( 'metas' )->updateMeta( 'og:description', strip_tags( $selected['title'] ) );
		}
		...
		<div class='catalog catalog_nomargin'>
			<div class='all_lines'>
				...
				<div class='main_data'><div class='md_inner'>
					".( $selected ? $selected['text'].$main->modules->gmi( "social" )->getShareWidget( "https://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'], "https://".$_SERVER['HTTP_HOST'].$mysql->settings['local_folder']."?".$this->dbinfo['local']."=".$r['link'], "https://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'], "https://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'], $main->templates->title ) : '' )."
				</div></div>
				<div class='clear'></div>
			</div>
		</div>
	...
	function getAdminScreen( &$selectedElement, $path )
	{
		...
		if( $query->gp( "createissue" ) && $query->gp( "process" ) ) {
			...
			$mysql->mu( "INSERT INTO `".$mysql->t_prefix."issues` VALUES(
				0,
				'".$title."',
			...
			$r = $mysql->mq( "SELECT `id` FROM `".$mysql->t_prefix."issues` WHERE 1 ORDER BY `id` DESC" );
			
			if( $r ) {
			
				if( !@opendir( ROOT_PATH."files/upload/issues/".$r['id'] ) ) {
					@mkdir( ROOT_PATH."files/upload/issues/".$r['id'] );
					@chmod( ROOT_PATH."files/upload/issues/".$r['id'], 0777 );
					@file_put_contents( ROOT_PATH."files/upload/issues/".$r['id']."/index.html", "Nothing here" );
				}
				if( !@opendir( ROOT_PATH."files/upload/issues/".$r['id']."_files" ) ) {
					@mkdir( ROOT_PATH."files/upload/issues/".$r['id']."_files" );
					@chmod( ROOT_PATH."files/upload/issues/".$r['id']."_files", 0777 );
					@file_put_contents( ROOT_PATH."files/upload/issues/".$r['id']."_files/index.html", "Nothing here" );
				}
			...
			$mysql->mu( "UPDATE `".$mysql->t_prefix."issues` SET
		...
		} else if( $query->gp( "turnissue" ) ) {
			
			$id = $query->gp( "turnissue" );
			$ep = $mysql->mq( "SELECT `view` FROM `".$mysql->t_prefix."issues` WHERE `id`=".$id );			
			if( $ep )
				$mysql->mu( "UPDATE `".$mysql->t_prefix."issues` SET `view`=".( $ep['view'] ? "0" : "1" )." WHERE `id`=".$id );
				
		} else if( $query->gp( "delete" ) ) {
			
			$id = $query->gp( "delete" );
			$ep = $mysql->mq( "SELECT * FROM `".$mysql->t_prefix."issues` WHERE `id`=".$id );			
			if( $ep ) {
				$mysql->mu( "DELETE FROM `".$mysql->t_prefix."issues` WHERE `id`=".$id );
				$dh = @opendir( "files/upload/issues/".$id."/" );
				while( $dh && ( $file = @readdir( $dh ) ) !== false )
					if( $file !== '.' && $file !== '..' )
	          			@unlink( "files/upload/issues/".$id."/".$file );
				@rmdir( ROOT_PATH."files/upload/issues/".$id );
				$dh = @opendir( "files/upload/issues/".$id."_files/" );
				while( $dh && ( $file = @readdir( $dh ) ) !== false )
					if( $file !== '.' && $file !== '..' )
	          			@unlink( "files/upload/issues/".$id."_files/".$file );
				@rmdir( ROOT_PATH."files/upload/issues/".$id."_files" );
			}
			
		}
		
		if( $query->gp( "createissue" ) ) {
			
			return $this->getExternalNewissue( $path );
			
		} else if( $query->gp( "edit" ) ) {
			
			return $this->getExternalEditissue( $path, $query->gp( "edit" ) );
			
		}
		
		$selectedElement = 1;
		$t = "
			<h1>Список статей для раздела «Полезная информация»</h1>
			
			<a href=\"".$mysql->settings['local_folder']."admin/".$path."/createissue\">Создать новую статью</a><br><br>
		...
		$counter = 0;
		$a = $mysql->mqm( "SELECT * FROM `".$mysql->t_prefix."issues` WHERE ".$where." ORDER BY `order` DESC, `id` ASC" );
		while( $r = @mysql_fetch_assoc( $a ) ) {
			$t .= "
				<tr class='list_table_element'".( !$r['view'] ? " style='background-color: #fcd9d9;'" : "" ).">
					...
					<td nowrap>
						<a href=\"".LOCAL_FOLDER."admin/".$path."/turnissue".$r['id']."\">".( $r['view'] ? "Выключить" : "Включить" )."</a><br>
	...
	function getExternalNewissue( $link )
	{
		...
		$inner = "
				...
				<p>
					Текст статьи:<br>
					<textarea name=\"text\" id=\"text\" rows=25></textarea>
				</p>
	...
	tinyMCE.init({
		...
		return "
				...
				<form issue=\"".$mysql->settings['local_folder']."admin/".$link."\" method=POST id='creatings' style='text-align: left;'>
					".$inner."
					<div style='width: 300px; border-top: 1px solid #aaa; padding-top: 5px;'>
						<input type=hidden name=\"process\" id=\"process\" value=\"0\" />
						<input type=hidden name=\"createissue\" id=\"createissue\" value=\"0\" />						
	...
	function getExternalEditissue( $link, $issueid )
	{
		global $query, $mysql, $lang, $main, $utils;
		
		$data = $mysql->mq( "SELECT * FROM `".$mysql->t_prefix."issues` WHERE `id`=".$issueid );
		if( !$data ) {
	...
	tinyMCE.init({
        ...
        plugin_smimage_directory : '/files/upload/issues/".$issueid."',
        plugin_smexplorer_directory : '/files/upload/issues/".$issueid."_files',
